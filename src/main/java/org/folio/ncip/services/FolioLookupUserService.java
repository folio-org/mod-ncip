...
public class FolioLookupUserService extends FolioNcipService implements LookupUserService {
	...
	public LookupUserResponseData performService(LookupUserInitiationData initData,
		...
		if (userId == null) {
			...
			Problem p = new Problem(new ProblemType(Constants.LOOKUP_USER_VALIDATION_PROBLEM),
					Constants.LOOKUP_USER_VALIDATION_PROBLEM, Constants.COULD_NOT_DETERMINE_USER, "");
			responseData.getProblems().add(p);
			return responseData;
		}
		try {
		...
		try {
			...
			ProblemType problemType = new ProblemType("");
			Problem p = new Problem(problemType, Constants.GENERAL_ERROR,
					Constants.LOOKUP_USER_FAILED + ": " + e.toString());
			responseData.setProblems(new ArrayList<Problem>());
			responseData.getProblems().add(p);
			return responseData;
		}
	...
	private LookupUserResponseData constructResponse(LookupUserInitiationData initData, JsonObject userDetails,
			String requesterAgencyId) throws Exception {
		...
		try {
			...
			if (initData.getUserAddressInformationDesired())
				responseData.getUserOptionalFields()
						.setUserAddressInformations(this.retrieveAddress(userDetails, requesterAgencyId));

			if (initData.getUserPrivilegeDesired()) {
				responseData.getUserOptionalFields()
						.setUserPrivileges(this.retrievePrivileges(userDetails, requesterAgencyId));
				responseData.getUserOptionalFields().getUserPrivileges()
						.add(this.retrieveBorrowingPrvilege(userDetails, requesterAgencyId));
			}
		} catch (Exception e) {
	...
	private NameInformation retrieveName(JsonObject jsonObject) {
		...
		if (personal.getString(Constants.PREFERRED_FIRST_NAME) != null
				&& !personal.getString(Constants.PREFERRED_FIRST_NAME).isEmpty())
	...
	private ArrayList<UserAddressInformation> retrieveAddress(JsonObject jsonObject, String agencyId) {
		...
		if (ncipProperties != null) {
			includePhyscialAddressSetting = ncipProperties
					.getProperty(agencyId.toLowerCase() + "." + Constants.RESPONSE_INCLUDES_PHYSICAL_ADDRESS);
			if (includePhyscialAddressSetting == null)
				includePhyscialAddressSetting = ncipProperties
						.getProperty(Constants.RESPONSE_INCLUDES_PHYSICAL_ADDRESS);
		}
		if (includePhyscialAddressSetting == null || includePhyscialAddressSetting.equalsIgnoreCase("false"))
			return list;
		JsonArray arrayOfAddresses = personal.getJsonArray("addresses");
		if (arrayOfAddresses == null || arrayOfAddresses.isEmpty())
			return list;
	...
	private UserAddressInformation retrieveEmail(JsonObject jsonObject, String agencyId) {
		...
		if (ncipProperties != null) {
			String emailStringConfigValue = ncipProperties
					.getProperty(agencyId.toLowerCase() + "." + Constants.EMAIL_STRING);
			if (emailStringConfigValue != null)
				emailString = emailStringConfigValue;
		}
	...
	private String retrieveAuthenticationInputTypeOf(LookupUserInitiationData initData,
			RemoteServiceManager serviceManager) throws Exception {
	...
	private String getBlockedMessage(String agencyId) {
		if (ncipProperties != null
				&& ncipProperties.getProperty(agencyId.toLowerCase() + "." + Constants.BLOCKED_CONFIG) != null)
			return ncipProperties.getProperty(agencyId.toLowerCase() + "." + Constants.BLOCKED_CONFIG);
		if (ncipProperties != null && ncipProperties.getProperty(Constants.BLOCKED_CONFIG) != null)
			return ncipProperties.getProperty(Constants.BLOCKED_CONFIG);
		return Constants.BLOCKED;
	}

	private String getOkMessage(String agencyId) {
		if (ncipProperties != null
				&& ncipProperties.getProperty(agencyId.toLowerCase() + "." + Constants.OK_CONFIG) != null)
			return ncipProperties.getProperty(agencyId.toLowerCase() + "." + Constants.OK_CONFIG);
		if (ncipProperties != null && ncipProperties.getProperty(Constants.OK_CONFIG) != null)
			return ncipProperties.getProperty(Constants.OK_CONFIG);
